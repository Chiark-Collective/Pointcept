...
class Block(PointModule):
    ...
    def forward(self, point: Point):
        ...
        try:
            ...
            print(f"ASSERTION FAILED in Block forward index: {self.cpe[0].indice_key}")
            # analyze_nans(point_dict["pooling_parent"]["feat"])
            raise
        else:
            print(f"ALL GOOD in Block forward index: {self.cpe[0].indice_key}: original point {point}")
        # add nan check
...
class PointTransformerV3(PointModule):
    ...
    def _create_encoder(
        ...
        for s in range(self.num_stages):
            ...
            stage = self._create_encoder_stage(
                s,
                enc_depths[s],
                enc_channels,
                enc_num_head[s],
                enc_patch_size[s],
                stride[s - 1] if s > 0 else None,
                enc_drop_path_,
                mlp_ratio,
                qkv_bias,
                qk_scale,
    ...
    def _create_decoder(
        ...
        for s in reversed(range(self.num_stages - 1)):
            ...
            dec_drop_path_.reverse()
            stage = self._create_decoder_stage(
                s,
                dec_depths[s],
                dec_channels[s],
                dec_channels[s + 1],
    ...
    #         print("Initial point.feat:", point.feat)

    #     point.serialization(order=self.order, shuffle_orders=self.shuffle_orders)
    #     print("After serialization point:", point)
    #     if hasattr(point, 'feat'):
    #         print("After serialization point.feat:", point.feat)

    #     point.sparsify()
    #     print("After sparsify point:", point)
    #     if hasattr(point, 'feat'):
    #         print("After sparsify point.feat:", point.feat)

    #     point = self.embedding(point)
    #     print("After embedding point:", point)
    #     if hasattr(point, 'feat'):
    #         print("After embedding point.feat:", point.feat)

    #     point = self.enc(point)
    #     print("After encoder point:", point)
    #     if hasattr(point, 'feat'):
    #         print("After encoder point.feat:", point.feat)

    #     if not self.cls_mode:
    #         point = self.dec(point)
    #         print("After decoder point:", point)
    #         if hasattr(point, 'feat'):
    #             print("After decoder point.feat:", point.feat)
